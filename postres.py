...
def connect():
    conn = psycopg2.connect(database="vladlink",
                            user="vladlink",
                            password="vladlink1",
                            host="127.0.0.1",
                            port=5432)
...
def drop_table():
    conn = connect()
    query = "DROP TABLE IF EXISTS vladlink"
    with conn:
        with conn.cursor() as cur:
            cur.execute(query)
    conn.commit()
...
def select_items(deep=100):
    if deep < 1:
        ...
        return
    conn = connect()
    query = f"SELECT id, parent, alias, name, level  FROM vladlink WHERE level<={deep}"
    with conn:
        with conn.cursor() as cur:
            cur.execute(query)
            data = cur.fetchall()
