...
def recursive_table(table, file, item, deep, level=0, url=''):
    ...
    file.write(line)
    if childrens:
        [recursive_table(table, file, child, deep, level, url) for child in childrens]


def table_to_file(table, filename, deep=100):
    if not table:
    ...
    with open(filename, 'w') as f:
        ...
        for item in items:
            recursive_table(table, f,  item, deep)
